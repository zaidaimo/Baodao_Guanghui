...
public class Simp_Spawn_Guest_1 : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        if (isGame == true)
        {
            InvokeRepeating("CreateGuests", spawnTime, 10);  //反覆呼叫 (1秒生成,每15秒生成一次)
        }
    }
    ...
    {
        InvokeRepeating("CreateGuests", spawnTime,15);  //反覆呼叫 (1秒生成,每1秒生成一次)
